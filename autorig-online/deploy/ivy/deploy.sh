#!/usr/bin/env bash
set -euo pipefail
base=/srv/autorig/webgl-assets/ivy
...
snippet=/etc/nginx/snippets/autorig-ivy-webgl.conf
mkdir -p "$base/deploy" "$base/releases" "$base/shared/ivy/Build"
...
test -f "$destination/ivy/index.html"
test -f "$destination/ivy/release.json"
python3 - "$destination/ivy" <<'PY'
...
PY
for file in "$destination/ivy/Build/"*; do
    name=$(basename "$file")
    target="$base/shared/ivy/Build/$name"
...
line='    include /etc/nginx/snippets/autorig-ivy-webgl.conf;'
...
curl --fail --silent --show-error --retry 5 --retry-all-errors --retry-delay 1 --compressed --resolve autorig.online:443:127.0.0.1 https://autorig.online/ivy/release.json > "$backup/live-release.json"
