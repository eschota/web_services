...
- Шлюз мульти-сайта: сырое тело запроса проксируется дальше по `product_permalink`: **`freestock-*`** → `FREESTOCK_GUMROAD_PROXY_TARGET` (по умолчанию `https://freestock.online/gumroad/ping`, переопределение через env), остальные продукты → `https://free3d.online/gumroad/ping`.
- Локальное начисление кредитов AutoRig только для продуктов `autorig-*` из маппинга в `config.py`; покупки Freestock только пересылаются на свой backend.
...
- **Канонический прод:** `deploy/nginx.conf`, `autorig.service` и скрипты деплоя используют только **`/root/autorig-online`**. Отдельная прод-копия не нужна: рабочий git checkout является живым деревом сервиса.
- **SQLite:** файл по умолчанию `backend/db/autorig.db` (относительно `WorkingDirectory`). **Нельзя подменять прод-БД пустой** (например, создав новый `backend/db/` при правках): в `telegram_chats` пропадут подписчики и сломаются и уведомления, и support forum (резолвер чата). Деплой через `rsync` с `--exclude db/` как в `push-and-deploy.sh`.
