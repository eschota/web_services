...
test('shared skeleton usage is unioned across multiple skinned meshes', () => {
    const root = bone('Root');
    const front = bone('FrontLeg', root);
    const rear = bone('RearLeg', root);
    const helper = bone('ControlOnly', root);
    const bones = [root, front, rear, helper];
    const mesh = (name, boneIndex) => ({
        name,
        isSkinnedMesh: true,
        skeleton: { bones },
        geometry: {
            attributes: {
                skinIndex: attribute([boneIndex, 0, 0, 0]),
                skinWeight: attribute([1, 0, 0, 0]),
            },
        },
    });
    const nodes = [root, front, rear, helper, mesh('FrontMesh', 1), mesh('RearMesh', 2)];
