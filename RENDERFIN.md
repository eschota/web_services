...
Поэтому падение туннелей не замедлило очередь, а **остановило** её: все
воркеры выглядели занятыми генерациями, которых никто не видел. Ноль
завершений за 75 минут при четырёх простаивающих боксах из пяти. Теперь
подряд идущие ошибки транспорта считаются, любой ответ счётчик сбрасывает, а
после порога слот освобождается и работа переотправляется без траты попытки.
