...
## Разбор: как деплой остановил очередь целиком (2026-08-03)

Три независимых дефекта сложились в полную остановку. Каждый по отдельности
был бы безобиден.

**1. Бит исполнения жил только на сервере.** `farm-tunnels.sh` лежал в git как
`100644`, тогда как все остальные скрипты в `deploy/` — `100755`. Пока служба
работала процессом, запущенным до чекаута, это ничего не значило. Деплой через
`git reset --hard` переписал файл из индекса, и первый же перезапуск юнита дал
`203/EXEC`. Туннели легли и подняться не могли. **Правило: скрипт, на который
ссылается systemd, проверяйте через `git ls-files -s`, а не по `ls` на
сервере — на сервере права могут быть выставлены руками и пережить что угодно,
кроме деплоя.**

**2. Обрыв связи держал слот воркера.** Опрос статуса глотал любую ошибку
транспорта и повторял до потолка в 4 часа. Задача, которая опрашивает
недоступный адрес, всё это время числится занимающей единственный слот бокса.
Поэтому падение туннелей не замедлило очередь, а **остановило** её: все пять
слотов были заняты генерациями, за которыми никто не мог следить, и ни одна
новая задача не отправлялась. Ноль завершений за 75 минут. Теперь подряд
идущие ошибки транспорта считаются, любой ответ счётчик сбрасывает, а после
порога слот освобождается и работа переотправляется без траты попытки.

**Чего в этом списке НЕТ — и это важно.** По дороге я решил, что боксы
простаивают с «занятыми» слотами, и добавил таймаут на статус `Pending`.
Это была ошибка чтения: поле `hunyuan.service_state` описывает только адаптер
Hunyuan, а не занятость машины. Боксы выполняли обычные пользовательские
конверсии по 18–80 минут (`processing_tasks`, `type: ConversionTask`), и
конвертер по устройству держит одну задачу за раз — наша честно ждала в
очереди. Изменение откачено: оно отбирало бы задачу у занятого бокса и
плодило на нём брошенные генерации, которые он потом всё равно посчитает и
отдаст в никуда. **Прежде чем называть бокс простаивающим, смотрите
`processing_tasks` и `tasks_summary`, а не `hunyuan.service_state`.**
Патологический случай уже покрыт четырёхчасовым потолком стадии.

Отсюда же ответ на вопрос «почему так медленно»: ферма общая. Пропускная
способность генераций — это то, что осталось от неё после пользовательских
конверсий, и в спокойном состоянии это порядка 4 задач в час на пяти боксах.

**3. Мониторинг всё увидел и промолчал.** Прогон healthcheck в 12:11 — прямо
посреди аварии — перечислил все девять проблем и закончил строкой
`notify skipped: no token/chat configured`. Токен был, чата не было. Детекция
работала идеально, доставки не существовало. **Healthcheck без
`HEALTHCHECK_CHAT_ID` — это не мониторинг, а лог.** Пишет он только при
наличии проблем, так что здоровая ферма молчит.
...
Что при этом **доезжает** до боевого пути: перед каждым запуском конвертер
копирует пакет `autorig_hunyuan` в
`C:\Users\user\AppData\Local\Temp\vbfast\` и запускает Blender уже оттуда.
Копия постадийная — на простаивающем боксе её просто нет, а на работающем её
mtime совпадает со стартом процесса запечки секунда в секунду. Поэтому правка
в `C:\3d\GLB_Convverter_Git\GLB_Convverter_WebServer\autorig_hunyuan\`
подхватывается **следующей** запечкой без всяких рестартов — но уже идущая
доедет по-старому. Не ищите свой файл в работающем процессе: смотрите на mtime
копии в `vbfast` против своего деплоя.
