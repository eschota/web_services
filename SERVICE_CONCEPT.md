...
- `POST /api/task/create` → `backend/main.py` → `backend/tasks.py:create_conversion_task(...)` + списание кредитов / anon usage.
  - Тело: **`application/json`** — достаточно `{"input_url":"https://..."}` (по умолчанию `source=link`, `type=t_pose`); либо **multipart** / form с теми же полями и опциональным `file` для upload.
